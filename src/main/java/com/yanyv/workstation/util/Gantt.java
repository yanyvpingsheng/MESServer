...
import com.yanyv.workstation.view.Color;
import com.yanyv.workstation.view.Label;
import com.yanyv.workstation.view.Line;
...
public class Gantt {
    ...
    // 静态方法绘制甘特图 传入容器 数据
    public static void drawGantt(Canvas pane, List<GanttValue> vList) {
        // 表宽
        ...
        for (int i = 0; i < vList.size(); i++) {
            ...
            for (int j = 0; j < vList.get(i).lineData.size(); j++) {
                ...
                label.setLayoutY(sideTop + i * lineHeight);
                label.setWidth(value.length * disX);
                label.setHeight(lineHeight - lineDis);
                label.setColor(colors.get(index));
                pane.getChildren().add(label);
            }
        }
    ...
    public static class GanttValue {
        ...
        public void formatToGanttValue(List<? extends GanttFormat> list, String mode) {
            // 将列表内容格式化为GanttItemValue对象，并添加到列表
            for (GanttFormat g : list) {
                GanttItemValue value = new GanttItemValue();
                value.id = g.getGanttId(mode);
                value.lineName = g.getLineName(mode);
    ...
    public interface GanttFormat {
        // 同色块需要相同id
        String getGanttId(String mode);
        // 行名称
        String getLineName(String mode);
